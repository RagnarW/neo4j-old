...
public class RaftMessageDistributor extends MessageToMessageEncoder<RaftMessages.ClusterIdAwareMessage>
{
    @Override
    protected void encode( ChannelHandlerContext ctx, RaftMessages.ClusterIdAwareMessage msg, List<Object> out ) throws Exception
    {
        out.add( new RaftMessageHeader( msg ) );
        if ( msg.hasContent() )
        {
            ReplicatedContent content = ((RaftMessages.ContentIncludedRaftMessage) msg.message()).content();
            assert content != null;
            out.add( content );
        }
    }
